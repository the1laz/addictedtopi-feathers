...
function addText(svg,pins) {
  ...
  .attr("dx",function(d,i){
    if (d3.select(this.parentNode.parentNode).datum().side == "left") {
      if (i==0){
        b = -12;
      }
      b = b - d.bbox.width - 8
      ...
      b = b + d.bbox.width + 8
      return b - d.bbox.width;
    }
    // console.log(d3.select(this.parentNode.parentNode));

  })

    //d3.select(this.parentNode.parentNode).datum().side
...
//d3.json('../'+get.board+'/board.json',function(error, response) {
  ...
  d3.select("body")
    .append("H1")
    .html(data.site + " " + data.type + " " + data.name);
